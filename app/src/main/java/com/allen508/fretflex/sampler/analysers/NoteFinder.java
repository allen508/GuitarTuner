...
public class NoteFinder implements SampleAnalyser {
    ...
    private static final int[] FREQUENCIES = { 329, 247, 196, 147, 110, 82};
    ...
    public AnalysisResult analyse(AnalysisResult result) {
        ...
        int index = getIndex((int)result.getFrequency());

        return new AnalysisResult(result.getBytes(), null, result.getFrequency(), index);

    }

    private int getIndex(int freq) {
        int distance = Math.abs(FREQUENCIES[0] - freq);
        int idx = 0;
        for (int c = 1; c < FREQUENCIES.length; c++) {
            int cdistance = Math.abs(FREQUENCIES[c] - freq);
